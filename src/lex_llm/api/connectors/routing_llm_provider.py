...
from .health_monitor import VLLMHealthMonitor
...
class RoutingLLMProvider(LLMProvider):
    """
    Prefers DGX; falls back to Scaleway when the DGX backend is overloaded
    or fails. Implements hysteresis to avoid flapping.
    """

    def __init__(
        self,
        primary: DGXProvider,
        fallback: ScalewayProvider,
        ...
        async with self._state_lock:
            if overloaded:
                ...
                if (
                    ...
                    logger.warning(f"Tripping to fallback (Scaleway): {reason}")
            else:
                ...
                if (
                    ...
                    self._using_fallback = False
                    logger.info("DGX recovered, routing back to primary")
        ...
        if backend is self.primary:
            ...
            try:
                ...
                if tokens_yielded > 0:
                    ...
                    logger.error(
                        f"DGX degraded mid-stream after {tokens_yielded} tokens: {e}. "
                        f"Cannot transparently fail over."
                    )
                    raise
                logger.warning(f"DGX failed before first token: {e}. Failing over.")
                ...
                if tokens_yielded > 0:
                    raise
                logger.exception(f"DGX hard failure: {e}. Failing over.")
